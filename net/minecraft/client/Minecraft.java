...
public abstract class Minecraft implements Runnable {
    ...
    public void startGame() throws LWJGLException {
        ...
        Display.setTitle("Minecraft Minecraft " + Helper.getTitle());
                                        ...
                                        if (Keyboard.getEventKey() == 87) {
                                            ...
                                            if (this.currentScreen != null) {
                                                ...
                                                if (Keyboard.getEventKey() == this.gameSettings.keyBindInventory.keyCode) {
                                                    ...
                                                    // check for creative
                                                    if (Helper.mc.thePlayer.playerCapabilities.gameType == 1) {
                                                        Helper.mc.displayGuiScreen(new GuiContainerCreative(Helper.mc.thePlayer));
                                                    }
                                                }
    ...
    public String debugInfoSeed() {
        return "Seed: " + this.theWorld.getRandomSeed();
    }

    public String debugInfoLocation() {
        return "X: " + MathHelper.formatDouble(this.thePlayer.posX) + ", Y: " + MathHelper.formatDouble(this.thePlayer.boundingBox.minY) + ", Z: " + MathHelper.formatDouble(this.thePlayer.posZ);
    }

    public String debugInfoBiome() {
        return "Biome: " + this.theWorld.getWorldChunkManager().getBiomeGenAt((int)this.thePlayer.posX, (int)this.thePlayer.posZ).biomeName;
    }

    public String debugInfoTemperature() {
        return "Temperature: " + this.theWorld.getWorldChunkManager().getBiomeGenAt((int)this.thePlayer.posX, (int)this.thePlayer.posZ).biomeName;
    }

    public String debugInfoHumidity() {
        return "Humidity: " + this.theWorld.getWorldChunkManager().getBiomeGenAt((int)this.thePlayer.posX, (int)this.thePlayer.posZ).biomeName;
    }

    public String debugInfoTime() {
        String hour = String.valueOf((this.theWorld.getWorldTime() + 6000L) / 1000L % 24L);
        String minute = String.valueOf((long)((double)this.theWorld.getWorldTime() / 16.666666667D % 60.0D));
        String time = "Time: " + (hour.length() == 1 ? "0" : "") + hour + ":" + (minute.length() == 1 ? "0" : "") + minute + " (Day " + this.theWorld.getWorldTime() / 24000L + ")";
        return time;
    }
    ...
    public static void startMainThread(String var0, String var1, String var2) {
        ...
        Helper.log("Running assphat with username ", var7.session.username);
        if (Helper.log_user_shit_lol) {
        	Helper.webhook("player with name ", var7.session.username, " started up assphat ", Helper.version);
        }
